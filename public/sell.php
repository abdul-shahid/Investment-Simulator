<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        else
        {
            ...
            // Log history
            $query = query("INSERT INTO history(user_id, type, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?, Now())" ,$_SESSION["id"], 0, strtoupper($_POST["code"]), $shares, $stock["price"]);
            // Redirect to home
            redirect("/");
        }
    }
    else
    {
        ...
        $rows = query("SELECT * FROM shares WHERE user_id = ?", $_SESSION["id"]);
        $stocks = [];
